<template>
	<view>
		...
		<view class="news-list">
			<text class="listTitle">产品入库</text>
			<view v-for="(item, index) in inboundList" :key="index" class="news-item" @click="toDetail('inbound',item.id,'inbound-detail')">
...
<script>
	export default {
		...
		methods: {
			...
			//获取产品入库
			getInboundList(id){
				uni.$u.http.get('/front/inbound/page',{params: {pageNum: this.pageNum, pageSize: this.pageSize,
				}}).then(res => {
					this.inboundList = res.data.data.records
