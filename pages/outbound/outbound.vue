<template>
	<view>
		...
		<view class="news-list">
			<text class="listTitle">产品出库</text>
			<view v-for="(item, index) in outboundList" :key="index" class="news-item" @click="toDetail('outbound',item.id,'outbound-detail')">
...
<script>
	export default {
		...
		onLoad(options) {
			...
			//轮播图
			this.getBannerList()
			this.getOutboundList(null,name)
		},
		methods: {
			//获取轮播图
			getBannerList(){
			...
			//获取产品出库
			getOutboundList(id){
				uni.$u.http.get('/front/outbound/page',{params: {pageNum: this.pageNum, pageSize: this.pageSize,
				}}).then(res => {
					this.outboundList = res.data.data.records
